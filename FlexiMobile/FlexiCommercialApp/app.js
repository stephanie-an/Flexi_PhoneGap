//<debug>
Ext.Loader.setPath({
    'Ext': 'touch/src',
    'Dummy': 'app'
});
//</debug>

Ext.application({

    requires: [
        'Ext.Container',
        'Ext.dataview.List',
        'Ext.MessageBox',
        'Ext.Panel',
        'Ext.tab.Panel',
        'Ext.TitleBar',
        'Ext.data.reader.Array'
    ],
    viewport: {
        autoMaximize: true
    },

    models: [
        'Content',
        'Dealer',
        'SalesStaff',
        'Rates',
        'Product',
        'Equipment',
        'Tax',
        'Quote',
        'RuntimeConfig'
    ],
    stores: [
        'LocalSalesStaffStore',
        'SalesStaffStore',
        'RatesStore',
        'ProductStore',
        'EquipmentStore',
        'TaxStore',
        'ContentStore',
        'LocalDealerStore',
        'LocalContentStore',
        'LocalEquipmentStore',
        'LocalProductStore',
        'LocalRatesStore',
        'LocalTaxStore',
        'LocalQuoteStore',
        'QuoteStore',
        'TickerStore',
        'LocalTickerStore',
        'RuntimeConfigStore'
    ],
    views: [
        'TabPanel',
        'Settings',
        'HomeContainer',
        'ResultPanel',
        'HistoryPanel',
        'CalculateInputPanel',
        'HistoryDisplayPanel',
        'HomePanel',
        'GenericDisplayPanel',
        'PromoTicker'
    ],
    icon: {
        57: 'resources/icons/Icon.png',
        72: 'resources/icons/Icon~ipad.png',
        114: 'resources/icons/Icon@2x.png',
        144: 'resources/icons/Icon~ipad@2x.png'
    },
    isIconPrecomposed: true,
    name: 'FlexiCommercial',
    startupImage: {
        '320x460': 'resources/loading/320x460.jpg',
        '640x920': 'resources/loading/640x920.png',
        '640x1096': 'resources/loading/640x1096.png',
        '768x1004': 'resources/loading/768x1004.png',
        '748x1024': 'resources/loading/748x1024.png',
        '1536x2008': 'resources/loading/1536x2008.png',
        '1496x2048': 'resources/loading/1496x2048.png'
    },
    controllers: [
        'AppController',
        'HomeController',
        'QuoteInputController',
        'ResultController',
        'HistoryController',
        'SettingsController'
    ],

    launch: function () {
        Ext.fly('appLoadingIndicator').destroy();

        Ext.create('FlexiCommercial.view.TabPanel', {fullscreen: true});
    },

    onUpdated: function() {
        Ext.Msg.confirm(
            "Application Update",
            "This application has just successfully been updated to the latest version. Reload now?",
            function(buttonId) {
                if (buttonId === 'yes') {
                    window.location.reload();
                }
            }
        );
    }

});
